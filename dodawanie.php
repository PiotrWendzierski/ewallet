		...
		else
		{
			if((isset($wszystko_ok))&& ($wszystko_ok == true))
			{
				...
				if($polaczenie->query("INSERT INTO transakcje VALUES (NULL,'$id', '$kategoria', '$cena', '$data', '0' )"))
				{
				 ...
				 //wkładanie do tabeli kategorie nazwy nowej kategorii
				 $rezultat = $polaczenie ->query("SELECT * FROM kategorie WHERE kategoria='$kategoria' AND id='$id'");
				 if(!$rezultat)throw new Exception($polaczenie->error);
				 $ilosc_kategorii = $rezultat->num_rows;
				 $_SESSION['ilosc_kategorii'] = $ilosc_kategorii;
				 //jesli nie było takiej kategorii to dodaj nazwe kategorii do bazy
				 if($ilosc_kategorii<1)
				 {
					 //dodawanie nazwy kategorii do bazy danych
					 if($polaczenie->query("INSERT INTO kategorie VALUES (NULL, '$kategoria', '$id', 1)"))
					{
						;
					  }
					else 
				 ...
				 //jeśli była to dodać jedną ilość transakcji do bazy
				 else
				 {
					 //odczytanie ilosc transakcji w danej kategorii ile ma dany uzytkownik
					 $sql = "SELECT ilosc_transakcji FROM kategorie WHERE id = $id";
					 if($rezultat = $polaczenie -> query ($sql))
					 {
						 $wiersz = $rezultat -> fetch_assoc();
						 $ilosc_transakcji = $wiersz['ilosc_transakcji'];
					 }
					 else 
					{
						throw new Exception($polaczenie->error);
					}
					$ilosc_transakcji = $ilosc_transakcji+1;
					 if($polaczenie->query("UPDATE kategorie SET ilosc_transakcji = $ilosc_transakcji WHERE id='$id'"))
					 {
						 ;
					 }
					else 
