...
<div id="container">
	...
	<div id="sortowanie">
		<form action="" method = "GET">
			...
			Cena do</br><input type="number" name = "cena_do" /> </br></br>
		
		</br></br>

			<input type="checkbox" name="wplywy" value="<?php if(isset($_GET['wplywy'])){echo $_GET['wplywy'] ;}?>"/> Wpływy </br>
			<input type="checkbox" name="wyplywy" value="<?php if(isset($_GET['wplywy'])){echo $_GET['wplywy'] ;}?>"> Wypływy </br></br></br></br>
		...
		else
		{
			...
			if ($ile_transakcji != 0)
			{
				...
				if(isset($_GET['od'] ))
				{
					...
					if($_GET['search'] == "") $query = "SELECT * FROM transakcje WHERE (data BETWEEN '$od' AND '$do') AND (id = '$id')AND (cena BETWEEN '$cena_od' AND '$cena_do')" ;
					else $query = "SELECT * FROM transakcje WHERE (data BETWEEN '$od' AND '$do') AND (id = '$id')AND (kategoria='$search')AND (cena BETWEEN '$cena_od' AND '$cena_do')" ;
					...
					{	
						if(!isset($_GET['wyplywy']))
						{
							...
							else $query = "SELECT * FROM transakcje WHERE (data BETWEEN '$od' AND '$do') AND (id = '$id')AND (cena>0) AND(kategoria='$search')AND (cena BETWEEN '$cena_od' AND '$cena_do')" ;
						}
						else
						{
							if($search == "")
